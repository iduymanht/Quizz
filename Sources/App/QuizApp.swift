...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        QuizController.shared.start()   // random quiz prompts while the pet is idle
    ...
    /// Handles `Quiz://link?token=…&login=…` — the tail of the web's
    /// GitHub sign-in, which links this app to the user's profile with no
    /// manual code entry.
    func application(_ application: NSApplication, open urls: [URL]) {
        for url in urls where url.scheme == "Quiz" && url.host == "link" {
