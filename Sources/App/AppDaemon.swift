...
final class AppDaemon: ObservableObject {
    ...
    private let server = EventSocketServer(path: QuizPaths.socketPath)
    private var pruneTimer: Timer?

    func start() {
        try? FileManager.default.createDirectory(
            atPath: QuizPaths.baseDir, withIntermediateDirectories: true
        )
        ...
        EventSocketServer.drainQueue(directory: QuizPaths.queueDir) { [store] event in
