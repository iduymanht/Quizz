...
/// Quiz's hooks, so install/uninstall refuse instead.
...
/// Installs/removes Quiz's hook entries in an agent's config. Claude Code,
...
public enum HookInstaller {
    ...
    static func isOurs(_ command: String) -> Bool {
        command.contains("Quiz") && command.contains("hook")
    }
    ...
    // {"Quiz": {Event: [{"hooks": [{"type": "command", "command": ...}]}]}}

    /// The hook-group key Quiz owns in an Antigravity hooks.json.
    public static let antigravityGroup = "Quiz"
    ...
    /// Extracts the Quiz binary path from a hook command like
    /// `"/path/to/Quiz" hook --agent opencode` (the first quoted token).
    ...
    static func opencodePlugin(binary: String) -> String {
        """
        // Quiz integration (auto-generated, safe to delete to uninstall).
        // Reports opencode session lifecycle to Quiz's menu bar app.
        const Quiz_BIN = \(jsString(binary))
        export const Quiz = async ({ directory }) => {
          const sid = "opencode:" + (directory || "default")
          const send = (state) => {
            try {
              Bun.spawn([Quiz_BIN, "hook", "--agent", "opencode",
    ...
    static func piExtension(binary: String) -> String {
        """
        // Quiz integration (auto-generated, safe to delete to uninstall).
        // Reports Pi session lifecycle to Quiz's menu bar app.
        ...
        export default function (pi) {
          const send = (state, ctx) => {
            try {
              ...
              const p = spawn(Quiz_BIN, ["hook", "--agent", "pi",
