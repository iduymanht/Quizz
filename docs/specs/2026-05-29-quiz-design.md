...
1. State daemon (trong app)
   ...
   - Lắng nghe trên một Unix domain socket local (vd `~/.Quiz/Quiz.sock`).
   - Nhận event JSON, cập nhật state, publish ra UI (Combine/`@Observable`).
   - Dọn session cũ (timeout / khi nhận event kết thúc).

2. Bridge / CLI helper
   - Binary nhỏ trong cùng repo: `Quiz hook --event <E> --session <id> [--project <path>] [--agent <kind>] [--message <m>]`.
   - Được agent gọi qua cơ chế hook của agent đó; helper serialize event JSON và gửi vào socket.
   - Không phụ thuộc app đang mở: nếu socket không có, ghi tạm vào file queue (`~/.Quiz/queue/`) để daemon đọc khi mở.
...
Claude Code (cài qua `settings.json` hooks gọi `Quiz hook ...`):
...
- v2: load pet từ thư mục user (`~/.Quiz/pets/`), cộng đồng PR pet vào repo (dex), importer cho pet pack tương thích.
