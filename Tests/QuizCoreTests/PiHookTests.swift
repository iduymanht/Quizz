...
/// through the `Quiz hook` CLI with explicit flags (like the opencode plugin).
final class PiHookTests: XCTestCase {
    ...
    func testPiSpec() {
        ...
        XCTAssertTrue(spec.settingsPath.hasSuffix("/.pi/agent/extensions/Quiz.ts"))
    ...
    func testPiExtensionContent() {
        let ts = HookInstaller.piExtension(binary: "/x/Quiz")
    ...
    func testPiExtensionInstallRoundTrip() throws {
        let spec = AgentHooks.spec(for: .pi)!
        let path = tmp("Quiz.ts")
        let cmd = "\"/x/Quiz\" hook --agent pi"
        try HookInstaller.installToDisk(command: cmd, path: path, events: spec.events, style: .piExtension)
        let source = try String(contentsOfFile: path, encoding: .utf8)
        XCTAssertTrue(source.contains("Quiz_BIN"))
        XCTAssertTrue(source.contains("/x/Quiz"))
