...
final class MultiAgentHookTests: XCTestCase {
    private let cmd = "\"/Applications/Quiz.app/Contents/MacOS/Quiz\" hook --agent cursor"
    ...
    func testCursorInstallShape() {
        ...
        XCTAssertTrue((stop?.first?["command"] as? String ?? "").contains("Quiz"))
    }
    ...
    func testWindsurfInstallShape() {
        ...
        let cmd = "\"/x/Quiz\" hook --agent windsurf"
    ...
    func testAntigravityInstallShape() {
        ...
        let cmd = "\"/x/Quiz\" hook --agent antigravity"
    ...
    func testOpencodeBinaryPathExtraction() {
        XCTAssertEqual(
            HookInstaller.binaryPath(fromCommand: "\"/Applications/Quiz.app/Contents/MacOS/Quiz\" hook --agent opencode"),
            "/Applications/Quiz.app/Contents/MacOS/Quiz")
    }

    func testOpencodePluginContent() {
        let js = HookInstaller.opencodePlugin(binary: "/x/Quiz")
    ...
    func testDiskRoundTripAllStyles() throws {
        let tmp = NSTemporaryDirectory() + "Quiz-test-\(UUID().uuidString)/"
        defer { try? FileManager.default.removeItem(atPath: tmp) }
        let cases: [(AgentKind, String)] = [(.cursor, "cursor.json"), (.windsurf, "windsurf.json"), (.opencode, "plugin/Quiz.js"), (.antigravity, "config/hooks.json")]
        for (kind, file) in cases {
            let spec = AgentHooks.spec(for: kind)!
            let path = tmp + file
            let command = "\"/Applications/Quiz.app/Contents/MacOS/Quiz\" hook --agent \(kind.rawValue)"
